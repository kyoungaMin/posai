...
export default function SalesChart({ recentSales }: SalesChartProps) {
  const maxSales = Math.max(...recentSales.map((d) => d.daily_total), 1);
    ...
    <div className="lg:col-span-2 bg-white p-5 md:p-8 rounded-4xl border border-orange-100 shadow-sm">
      <div className="flex justify-between items-center mb-8">
        ...
        <span className="text-[10px] font-bold bg-orange-50 text-orange-600 px-3 py-1 rounded-full">
          최근 {recentSales.length}일
        </span>
      </div>
      {recentSales.length === 0 ? (
        <div className="h-48 md:h-64 flex items-center justify-center text-sm text-slate-400 font-medium">
          아직 매출 데이터가 없습니다.
        </div>
      ) : (
        <div className="h-48 md:h-64 flex items-end gap-2 md:gap-4 px-2 md:px-4">
          {recentSales.map((day, i) => {
            const barHeight = Math.max((day.daily_total / maxSales) * 100, 4);
            return (
              <div key={i} className="flex-1 flex flex-col items-center justify-end h-full group">
                <div className="relative w-full flex-shrink-0" style={{ height: `${barHeight}%` }}>
                  <div className="w-full h-full bg-orange-100 rounded-t-xl md:rounded-t-2xl transition-all group-hover:bg-orange-400" />
                ...
                <span className="text-[8px] md:text-[10px] font-black text-slate-300 uppercase mt-2 md:mt-3 flex-shrink-0">
